...
想先和大家分享下我养第一只龙虾的**经验总结**～希望对大家有帮助！
...
火山引擎入口：[一键部署OpenClaw/ClawdBot-火山引擎](https://www.volcengine.com/activity/clawdbot)

火山引擎教程：[快速部署OpenClaw(原Moltbot)，集成飞书AI助手--云服务器-火山引擎](https://www.volcengine.com/docs/6396/2189942?lang=zh)
...
阿里云入口：[OpenClaw - 9.9元定制7*24 AI助理 - 阿里云](https://www.aliyun.com/benefit/scene/moltbot?spm=5176.42028462.J_E6lQbksm_Obit3Bf0Tx1p.d_primary.5421154aDDyM3E&scm=20140722.M_10976299.P_177.MO_5556-ID_10963004-MID_10963004-CID_37520-ST_15696-V_1)

阿里云教程：[部署OpenClaw镜像并构建钉钉AI员工](https://help.aliyun.com/zh/simple-application-server/use-cases/quickly-deploy-and-use-openclaw?spm=5176.454194655176.J_dO0ipO37JZSEbBHtde_Cd.1.2d8f545bw4Fv88)
...
> Windows的朋友们可以参考：[awesome-openclaw-tutorial/docs/01-basics/02-installation.md](https://github.com/xianyu110/awesome-openclaw-tutorial/blob/main/docs/01-basics/02-installation.md)
...
感兴趣的朋友可以研究下：[awesome-openclaw-tutorial/docs/01-basics/02-installation.md](https://github.com/xianyu110/awesome-openclaw-tutorial/blob/main/docs/01-basics/02-installation.md)
...
有一个国内版Openclaw的教程，感兴趣的朋友可以研究下：[awesome-openclaw-tutorial/docs/01-basics/02-installation.md](https://github.com/xianyu110/awesome-openclaw-tutorial/blob/main/docs/01-basics/02-installation.md)
...
复制以下代码，替换弹窗中原有的JSON内容，点击下一步，确认新增权限，继续申请开通，确认后等待权限导入完成。
...
（如对话时未出现可直接跳过本步骤）
首次对话如果提示配对（Paring）请求，可以复制机器人回复的最后一行命令行，粘贴至终端并运行。
...
相当于启动时加载 AI 的"系统说明书"。
通常 BOOT.md 里会放：系统提示词、初始化配置、Agent 行为规则
...
**1password**
对接 1Password 密码管理器，用来读取密码、自动填充凭证、访问 API key。
⚠️注意安全问题
...
**obsidian** - 对接 Obsidian 笔记库（通过 obsidian-cli）

适合做：知识库自动整理、自动写入笔记、Agent 记忆系统
...
封装为独立模块：lib/reviewEngine.js

---

## 3️⃣ 自然语言管理入口

实现自然语言解析模块：lib/nlpParser.js
...
URL → 抓取 → Readability 提取正文 → LLM 生成摘要与标签 → Markdown 写入

### 每日回顾流程

cron 触发 → 扫描 status = inbox → 筛选 nextReviewAt <= 当前时间 → 推送飞书 → 更新 reviewCount → 重新计算 nextReviewAt
...
- 多源：https://github.com/hesamsheikh/awesome-openclaw-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx-source-tech-news-digest.md
- 油管：https://github.com/hesamsheikh/awesome-openclaw-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx-youtube-digest.md
- Reddit：https://github.com/hesamsheikh/awesome-openclaw-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx-reddit-digest.md

## 日程管理

视频里表述有误，Openclaw的bot可以直接修改日历，无需skill。
apple-reminders是创建 / 管理 Apple 提醒事项。直接安装就行！
...
> 📅 文档同步时间：2026-07-25 06:00 (Asia/Shanghai)
> 📄 来源：https://ycnezwebj31p.feishu.xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
> 📝 作者：Xuan酱 | revision_id: 2844
